...
function loadNotifications() {
  ...
  function refreshNotifications() {
    const allItems = [];
    Object.values(notificationSectionData).forEach((section) => {
    ...
    .map((item) => {
      ...
      return `
        <div class="notification-item" data-notif-id="${item.id || ""}">
          <button class="notification-close" onclick="removeNotification('${item.id || ""}')">×</button>
...
function removeNotification(notificationId) {
  ...
  try {
    const userPath = auth.currentUser.uid;
    remove(ref(database, `notifications/${userPath}/${notificationId}`));
