...
async function submitContactReply() {
    ...
    try {
        const recipientPath = getContactReplyTarget(entry);
        await push(ref(database, `notifications/${recipientPath}`), {
...
async function acceptEntry() {
    ...
    try {
        ...
        if (currentViewEntry.userId) {
            await push(ref(database, `notifications/${currentViewEntry.userId}`), {
...
async function sendPersonalNotif() {
    ...
    if (!currentViewEntry.userId) {
        await showErrorModal('Not Available', 'No user ID for this entry.');
        return;
    }
