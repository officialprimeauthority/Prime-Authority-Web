...
const allowedOrigins = (process.env.CORS_ORIGIN || 'http://localhost:3000,http://127.0.0.1:3000,http://localhost:5001,http://127.0.0.1:5001')
  .split(',')
...
app.use((req, res, next) => {
  ...
  const isAllowedOrigin = !origin || allowedOrigins.includes(origin.replace(/\/$/, '')) || origin.startsWith('http://localhost:') || origin.startsWith('http://127.0.0.1:');
...
app.post('/forgot-password', async (req, res) => {
  try {
    ...
    const actionCodeSettings = {
      url: process.env.FRONTEND_RESET_URL || 'http://127.0.0.1:5500',
